...
let description: string = ` This is First Line
This is Second Line
This is Third Line` // ** can use template string for multiline string use ` ` instead of backquilte
let anothorMessage: string = `I'm Bank` // ** can use template string for single quote string use ` ` instead of backquilte

// number type
let age: number = 21; // ** number type can be integer or float 
let height: number = 175.5; // ** number type can be integer or float  
let binary: number = 0b111001; // ** number type can be binary
let octal: number = 0o436; // ** number type can be octal
let hex: number = 0xadf0d; // ** number type can be hexadecimal

// boolean type
let isSingle: boolean = true; // ** boolean type can be true or false
let isMarried: boolean = !isSingle; // ** can use logical operator and &&, or ||, not !
...
console.log(unknownType); // ** can call string method on any type
 ...
 // null type
 let nullType : null = null; // ** null type can be null only
// nullType = "null"; // ** can not reassign a value to null type

// undefined type
let undefinedType : undefined = undefined; // ** undefined type can be undefined only
...
// union type
let unionType : string | boolean = "Hello World"; // ** union type can be string or boolean
